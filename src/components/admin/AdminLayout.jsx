...
export default function AdminLayout() {
  ...
  return <div className="min-h-screen bg-[#080a0f] text-slate-100 admin-theme">
    ...
    <motion.aside initial={false} className={`fixed inset-y-0 left-0 z-40 w-72 border-r border-white/10 bg-[#0d1017]/95 backdrop-blur-xl transition-transform lg:translate-x-0 ${open ? "translate-x-0" : "-translate-x-full"}`}>
      <div className="flex h-20 items-center justify-between border-b border-white/10 px-6"><NavLink to="/admin/dashboard" className="flex items-center gap-3"><span className="grid h-9 w-9 place-items-center rounded-xl bg-orange-500 font-black text-white">C</span><div><p className="font-semibold">Marketplace Control</p><p className="text-xs text-slate-500">Administration</p></div></NavLink><button onClick={() => setOpen(false)} className="lg:hidden"><X /></button></div>
